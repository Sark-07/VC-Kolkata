...
import StudentNotificationFetch from './StudentDashboardComponents/StudentNotificationFetch';
const url = 'http://localhost:3000/upload';

const NotificationFetchForm = ({
  setCourse,
  setDepartment,
  setSemester,
  setSession,
  ...
  return (
    <form
      className='notification-form common-form'
      onSubmit={(e) => fetchNotification(e)}
    >
      <div className={`${baseUrl == 'teacher-dashboard' && 'hide'} semester`}>
      ...
      <div
        className={`${baseUrl == 'teacher-dashboard' && 'hide'} course-type`}
      >
...
const Notification = () => {
  ...
  const fetchNotification = async (e) => {
    e.preventDefault();
    if (baseUrl == 'teacher-dashboard' && session) {
      navigate(`/${baseUrl}/notification/fetch?session=${session}`);
    } else if (
      ...
      navigate(
        `/${baseUrl}/notification/fetch?session=${session}&semester=${semester}&course=${course}&department=${department}`
      );
    } else {
      toast.error('Please fill up all fields.');
    }
  ...
  useEffect(() => {
    if (baseUrl == 'teacher-dashboard' && querySession) {
  ...
  }, [querySession, queryCourse, queryDepartment, querySemester]);

  return (
    <div className='common notification'>
      <h1>Notification</h1>
      {pathname == '/teacher-dashboard/notification' ||
      pathname == '/student-dashboard/notification' ? (
        <NotificationFetchForm
          setCourse={setCourse}
        ...
        pathname == '/teacher-dashboard/notification/fetch' ? (
        ...
        pathname == '/student-dashboard/notification/fetch' ? (
        <StudentNotificationFetch />
      ) : (
        <></>
      )}
    </div>
